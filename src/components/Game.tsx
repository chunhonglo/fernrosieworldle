...
export function Game({ settingsData, updateSettings }: GameProps) {
  ...
  const random_int = Math.floor(seedrandom.alea(dayString)() * 6);
  // var country2:Country; 
  const guesses = todays.guesses;
  // const {country2, guesses } = todays;
  const countrySelection = countriesWithImage
  const pickedCountry = countrySelection[random_int];
  // const pickedCountry = countrySelection[Math.floor(Math.random()*countrySelection.length)];
    ...
    <div className="flex-grow flex flex-col mx-2">
      ...
      <div className="flex my-1">
        ...
        <img
          ...
          src={`images/countries/${country?.code.toLowerCase()}/IMG_${random_int}.jpeg`}
          style={
